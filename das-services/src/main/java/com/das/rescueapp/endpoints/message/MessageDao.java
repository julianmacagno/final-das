...
public class MessageDao extends Dao<Message, Message> {
    ...
    public Message make(ResultSet resultSet) throws SQLException {
        ...
        if (this.hasColumn(resultSet, "isFromUser")) {
            message.setIsFromUser(resultSet.getBoolean("isFromUser"));
        }
        return message;
    }
    ...
    public List<Message> getNotSynchronizedMessages() throws SQLException {
        this.connect();
        this.setProcedure("GetNotSynchronizedMessages");
        List<Message> messageList = this.executeQuery();
        this.close();
        return messageList;
    }

    public void markAsSynchronized(Long entityId) throws SQLException {
        try {
            this.connect();

            this.setProcedure("MarkAsSynchronized(?)");
            this.setParameter(1, entityId);
